#!/usr/bin/env python3
"""
Myfxbook Earning System - ตัวเปิดโปรแกรมแบบคลิกเดียว

ใช้ Python 3.8 ขึ้นไป ไม่ต้องติดตั้งไลบรารีเพิ่ม
เปิดใช้ครั้งแรกจะอยู่ในโหมดสาธิต ใช้งานได้เลยโดยไม่ต้องต่ออินเทอร์เน็ต
    ...
    python start.py --sync       ซิงก์ข้อมูลหนึ่งครั้งแล้วเปิดโปรแกรม
...
BANNER = """
  ╔══════════════════════════════════════════════════════════════╗
  ║   M Y F X B O O K   E A R N I N G   S Y S T E M              ║
  ║   พอร์ต · กำไรรายวัน · ส่วนแบ่ง HWM · จ่ายเงิน · เป้าหมาย        ║
  ╚══════════════════════════════════════════════════════════════╝
"""
...
def main():
    if sys.version_info < MIN_PYTHON:
        print("ต้องใช้ Python %d.%d ขึ้นไป (เครื่องนี้เป็น %s)"
              % (MIN_PYTHON[0], MIN_PYTHON[1], sys.version.split()[0]))
        input("กด Enter เพื่อปิด...")
        return 1

    parser = argparse.ArgumentParser(description="Myfxbook Earning System")
    ...
    parser.add_argument("--sync", action="store_true", help="ซิงก์ข้อมูลหนึ่งครั้งก่อนเปิด")
    ...
    if fresh:
        print("  กำลังสร้างฐานข้อมูลและพอร์ตตัวอย่างครั้งแรก...")

    if args.sync:
        from app.sync import sync_now
        init_db()
        result = sync_now()
        print("  %s" % (result.get("message") or result.get("error")))
